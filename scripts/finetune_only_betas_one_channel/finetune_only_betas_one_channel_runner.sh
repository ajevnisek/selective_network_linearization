...
then
  python3 snl_finetune_only_betas_one_channel.py $DATASET $ARCH $OUTDIR $INITIAL_CHECKPOINT \
    --block_type LearnableAlpha  --layer_index $LAYER_INDEX --channel_index $CHANNEL_INDEX \
  ...
  python3 snl_finetune_only_betas_one_channel.py $DATASET $ARCH $OUTDIR $INITIAL_CHECKPOINT \
    --block_type LearnableAlpha --beta_epochs $BETA_EPOCHS --lr_beta $LR_BETA --relu_budget $RELU_BUDGET \
