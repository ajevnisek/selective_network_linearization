...
for layer_name in ['layer1', 'layer2', 'layer3', 'layer4']:
    for block in [0, 1]:
        for relu_idx in [1, 2]:
            ...
            # if layername in ['layer1_block0_relu2', 'layer1_block1_relu2']:
            #     print(f"skipping {layername}...")
            #     continue
            activation_in = {}
            exec(
            ...
            original_acc = model_inference(base_classifier, test_loader,
                                           device, display=True)
            print("Original Model Test Accuracy: {:.5}".format(original_acc))
            exec(f'nof_activated_channels = np.unique(np.where((base_classifier.{layer_name}[{block}].alpha{relu_idx}.alphas > my_args.threshold).cpu())[1])')
            ...
            exec(f'snl_prototypes_indices = np.where((base_classifier.{layer_name}[{block}].alpha{relu_idx}.alphas > my_args.threshold).cpu())[1:]')
            ...
            os.makedirs(os.path.join(root, layername, ), exist_ok=True)
            plt.savefig(os.path.join(root, layername, 'correlation_mat.png'))

            original_cosine_similarity = cosine_similarity.copy()
            print(f"layer {layername} has {original_cosine_similarity.shape[0]} SNL prototypes")
            ...
            exec(f'alphas = base_classifier.{layer_name}[{block}].alpha{relu_idx}.alphas.cpu().detach()')
            # base_classifier.layer1[0].alpha1 = InducedLearnableAlpha(alphas.cuda(), inducers_mask.cuda(), sparse_matrix.cuda())
            exec(f'base_classifier.{layer_name}[{block}].alpha{relu_idx} = InducedLearnableAlpha(alphas.cuda(), inducers_mask.cuda(), sparse_matrix.cuda())')
            original_acc = model_inference(base_classifier, test_loader,
                                           device, display=True)
            print("Original Model Test Accuracy: {:.5}".format(original_acc))
