...
def generate_runai_command(job_index, export_filename):
	return f"""runai submit --name vladi-onr-greedier-{job_index} --gpu 1.0 -e EXPORT_SCRIPT=/storage/vladi/snl/optimize_and_reduce/scripts/{export_filename} -e RUN_SCRIPT=/storage/vladi/snl/optimize_and_reduce/scripts/optimize_and_reduce_greedier_runner.sh  -i ajevnisek/snl-amir:v6 --pvc=storage:/storage --large-shm"""


def generate_files():
	runai_bash_lines = []
	counter = 0
	for reduce_threshold in [0.3, 0.4, 0.5]:
		for step_reduce in [100, 200, 500, 1000, 1500, 2000]:
			for random_tries in [20, 50, 100]:
				for relu_target in [9000, 15000, 20000]:
					filename = f"relu_target_{relu_target}-step_reduce_{step_reduce}-random_tries_{random_tries}-reduce_threshold_{reduce_threshold}_greedier.sh"
					...
					counter += 1
	with open('scripts/optimize_and_reduce/runai_bash_lines_greedier.sh', 'w') as f:
