...
class WRN228ReLUInfo:
    ...
    def __init__(self, model):
        ...
        for block in ['block1', 'block2', 'block3']:
            for layer in [0, 1, 2]:
                for relu_idx in [1, 2]:
                    B, C, H, W = model.get_submodule(block).get_submodule('layer')[layer].get_submodule(f'alpha{relu_idx}').alphas.shape
...
def apply_alphas(snl_model, alpha_list):
    ...
    for block in ['block1', 'block2', 'block3']:
        for layer in [0, 1, 2]:
            for relu_idx in [1, 2]:
                snl_model.get_submodule(block).get_submodule('layer')[layer].get_submodule(f'alpha{relu_idx}').alphas.data = alpha_list[alpha_list_index].clone()
                alpha_list_index += 1
...
def get_flattened_alphas(snl_model):
    ...
    for block in ['block1', 'block2', 'block3']:
        for layer in [0, 1, 2]:
            for relu_idx in [1, 2]:
                alpha_list.append(snl_model.get_submodule(block).get_submodule('layer')[layer].get_submodule(f'alpha{relu_idx}').alphas.flatten())
...
def apply_alpha_long_tensor(snl_model, alpha_flattened_tensor):
    ...
    for block in ['block1', 'block2', 'block3']:
        for layer in [0, 1, 2]:
            for relu_idx in [1, 2]:
                B, C, H, W = snl_model.get_submodule(block).get_submodule('layer')[layer].get_submodule(f'alpha{relu_idx}').alphas.shape
                flattened_shape = C * H * W
                snl_model.get_submodule(block).get_submodule('layer')[layer].get_submodule(f'alpha{relu_idx}').alphas.data = alpha_flattened_tensor[ : flattened_shape].reshape(B, C, H, W).clone()
...
def optimize_and_reduce(snl_model, teacher_model, train_loader, test_loader, relu_target, random_tries, 
    ...
    info_gen = WRN228ReLUInfo(snl_model)
